...
module.exports = {
  list(req, res) {
    return tbl_suscriptores
      ...
      .then((tbl_suscriptores) => res.status(200).send(tbl_suscriptores))
  ...
  getById(req, res) {
    return tbl_suscriptores
      ...
      .then((tbl_suscriptores) => {
        if (!tbl_suscriptores) {
          return res.status(404).send({
            message: 'tbl_suscriptores Not Found',
          });
        }
        return res.status(200).send(tbl_suscriptores);
      })
      .catch((error) => res.status(400).send(error));
  },
