...
module.exports = {
  list(req, res) {
    return tbl_precios_boletas
      .findAll({
        ...
        order: [
          ['createdAt', 'DESC'],
          //[{ model: tbl_precios_boletas, as: 'vestuarios' }, 'createdAt', 'DESC'],
        ],
      })
      .then((tbl_precios_boletas) => res.status(200).send(tbl_precios_boletas))
  ...
  getById(req, res) {
    return tbl_precios_boletas
      ...
      .then((tbl_precios_boletas) => {
        if (!tbl_precios_boletas) {
          return res.status(404).send({
            message: 'tbl_precios_boletas Not Found',
          });
        }
        return res.status(200).send(tbl_precios_boletas);
      })
      .catch((error) => res.status(400).send(error));
  },
