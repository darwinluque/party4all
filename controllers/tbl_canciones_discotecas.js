...
const tbl_canciones_discotecas = require('../models').tbl_canciones_discotecas;
...
module.exports = {
  list(req, res) {
    return tbl_canciones_discotecas
      ...
      .then((tbl_canciones_discotecas) => res.status(200).send(tbl_canciones_discotecas))
  ...
  getById(req, res) {
    return tbl_canciones_discotecas
      ...
      .then((tbl_canciones_discotecas) => {
        ...
        return res.status(200).send(tbl_canciones_discotecas);
      })
      .catch((error) => res.status(400).send(error));
  },
  ...
  add(req, res) {
    return tbl_canciones_discotecas
      .create({
        ...
        str_genero: req.body.str_genero, 
        str_artista: req.body.str_artista, 
    ...
    return tbl_canciones_discotecas
      ...
      .then(tbl_canciones_discotecas => {
        ...
        return tbl_canciones_discotecas
          ...
          .then(() => res.status(200).send(tbl_canciones_discotecas))
          .catch((error) => res.status(400).send(error));
      })
      .catch((error) => res.status(400).send(error));
  },
...
function msgerror(error){
  ...
  if(error.includes('is not present in table \"tbl_canciones\"'))
    return JSON.parse('{"code": "5", "message": "ERROR: La cancion no se encuentra creada"}');
