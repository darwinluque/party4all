...
module.exports = {
  list(req, res) {
    return tbl_personas_discoteca
      ...
      .then((tbl_personas_discoteca) => res.status(200).send(tbl_personas_discoteca))
  ...
  getById(req, res) {
    return tbl_personas_discoteca
      ...
      .then((tbl_personas_discoteca) => {
        if (!tbl_personas_discoteca) {
          return res.status(404).send({
            message: 'tbl_personas_discoteca Not Found',
          });
        }
        return res.status(200).send(tbl_personas_discoteca);
      })
      .catch((error) => res.status(400).send(error));
  },
  ...
  add(req, res) {
    return tbl_personas_discoteca
      ...
      .then((tbl_personas_discoteca) => res.status(201).send(tbl_personas_discoteca))
      .catch((error) => res.status(400).send(error));
  },
    ...
    return tbl_personas_discoteca
      ...
      .then(tbl_personas_discoteca => {
        ...
        return tbl_personas_discoteca
          ...
          .then(() => res.status(200).send(tbl_personas_discoteca))
          .catch((error) => res.status(400).send(error));
      })
      .catch((error) => res.status(400).send(error));
  },
